...
#  */
# /**
#  * @Information  The Nginx Engine was made from the Engintron for cPanel/WHM plugin but was
#  *   reengaged to so that you don't have to disable the php open_basedir Protection and is
#  *   compiled with a newer version of OpenSSL so that nginx can use http/2, ALPN, NPN and OCSP stapling.
#  */
...
function install_nginxengine_ui {

    echo "=== Installing NginxEngine WHM plugin files... ==="

    # Cleanup older installations from the obsolete addon_nginxengine.cgi file
    if [ -f $CPANEL_PLG_PATH/addon_nginxengine.cgi ]; then
        /bin/rm -f $CPANEL_PLG_PATH/addon_nginxengine.cgi
    fi

    /bin/cp -f $APP_PATH/app/nginxengine.php $CPANEL_PLG_PATH/
    chmod +x $CPANEL_PLG_PATH/nginxengine.php

    echo ""
    echo "=== Register NginxEngine as a cPanel app ==="

    /usr/local/cpanel/bin/register_appconfig $APP_PATH/app/nginxengine.conf
...
function remove_nginxengine_ui {

    echo "=== Removing NginxEngine WHM plugin files... ==="
    /bin/rm -f $CPANEL_PLG_PATH/nginxengine.php

    echo ""
    echo "=== Unregister NginxEngine as a cPanel app ==="

    /usr/local/cpanel/bin/unregister_appconfig nginxengine
...
install)
    ...
    if [ ! -f /nginxengine.sh ]; then
        echo ""
        echo ""
        echo "***********************************************"
        echo ""
        echo " NGINXENGINE NOTICE:"
        echo " You must place & execute nginxengine.sh"
        echo " from the root directory (/) of your server!"
        echo ""
        echo " --- Exiting ---"
        echo ""
    ...
    echo "**************************************"
    echo "*        Installing NginxEngine        *"
    echo "**************************************"

    echo ""
    echo ""

    chmod +x /nginxengine.sh

    if [[ $2 == 'local' ]]; then
        echo -e "\033[36m=== Performing local installation from $APP_PATH... ===\033[0m"
        cd /
    else
        # Set NginxEngine src file path
        if [[ ! -d $APP_PATH ]]; then
            mkdir -p $APP_PATH
        fi
        ...
        # Get the files
        cd $APP_PATH
        wget --no-check-certificate -O nginxengine.zip https://github.com/engintron/engintron/archive/master.zip
        unzip nginxengine.zip
        /bin/cp -rf $APP_PATH/engintron-master/* $APP_PATH/
        /bin/rm -rvf $APP_PATH/engintron-master/*
        /bin/rm -f $APP_PATH/engintron.zip
        cd /
    ...
    if [ -f $APP_PATH/nginxengine.sh ]; then
        chmod +x $APP_PATH/nginxengine.sh
        $APP_PATH/nginxengine.sh purgecache
    fi
...
remove)
    ...
    echo ""
    echo "=== Removing NginxEngine files... ==="
    /bin/rm -rvf $APP_PATH
...
-h|--help|*)
    ...
    echo "    _   _____________   _____    ___    _______   _____________   ________ "
    echo "   / | / / ____/  _/ | / /\  \  /  /   / ____/ | / / ____/  _/ | / / ____/ "
    echo "  /  |/ / / __ / //  |/ /  \  \/  /   / __/ /  |/ / / __ / //  |/ / __/    "
    echo " / /|  / /_/ // // /|  /   /  /\  \  / /___/ /|  / /_/ // // /|  / /___    "
    echo "/_/ |_/\____/___/_/ |_/   /__/  \__\/_____/_/ |_/\____/___/_/ |_/_____/    "
    echo "                                                        "
    echo "                  https://www.my-hosted-website.com                 "
    cat <<EOF
NginxEngine for cPanel/WHM is the easiest way to integrate Nginx on your cPanel/WHM server.
Usage: /engintron.sh [command] [flag]
Main commands:
    install          Install, re-install or update NginxEngine (enables Nginx and compiled with a newer version of OpenSSL).
                     Add optional flag "mainline" to install Nginx mainline release.
    remove           Remove NginxEngine completely.
    enable           Set Nginx to ports 80/443 & Apache to ports 8080/8443
    disable          Set Nginx to ports 8080/8443 & switch Apache to ports 80/443
    purgecache       Purge Nginx's "cache" & "temp" folders,
                     then restart both Apache & Nginx
